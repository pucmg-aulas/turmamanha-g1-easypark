...
public class PagamentobdDAO {
    ...
    public Map<String, Map<String, Double>> buscarTempoMedioEOcupacao(int idEstacionamento) throws SQLException {
        String sql = """
        SELECT 
    ...
    """;
         Map<String, Map<String, Double>> relatorio = new HashMap<>();
    ...
    try (Connection conn = BancoDados.getConexao(); PreparedStatement ps = conn.prepareStatement(sql)) {
        ...
        try (ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                String tipoVaga = rs.getString("TipoVaga");
                double tempoMedio = rs.getDouble("TempoMedioOcupacao");
                double faturamentoMedio = rs.getDouble("FaturamentoMedioPorUso");

                Map<String, Double> dados = new HashMap<>();
                dados.put("TempoMedioOcupacao", tempoMedio);
                dados.put("FaturamentoMedioPorUso", faturamentoMedio);

                relatorio.put(tipoVaga, dados);
            }
        }
    } catch (SQLException e) {
        throw new SQLException("Erro ao buscar o tempo médio e ocupação por tipo de vaga: " + e.getMessage());
    }
